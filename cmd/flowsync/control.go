...
func purgeCommand(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if *project {
		...
		isLocal := target == filepath.Join(resolvedWorkingDir, ".flowsync")
		if isLocal {
			localJournalDir = filepath.Join(resolvedWorkingDir, "flowsync")
		} else {
	...
	if localJournalDir != "" {
		_ = os.RemoveAll(localJournalDir)
	}
