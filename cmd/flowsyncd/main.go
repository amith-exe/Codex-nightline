...
import (
	...
	"flowsync/internal/buildinfo"
	"flowsync/internal/core"
	"flowsync/internal/daemon"
	"flowsync/internal/ipc"
	"flowsync/internal/journal"
	"flowsync/internal/reflector"
)
...
func (h checkpointHandler) Handle(ctx context.Context, result daemon.IngestResult) error {
	...
	if strings.TrimSpace(result.Excerpt) != "" {
		// Destination dir: explicit flag wins, otherwise use the project's checkpoint log directory.
		destDir := h.checkpointLogDir
		if strings.TrimSpace(destDir) == "" {
			if logDir, err := h.store.CheckpointLogDir(checkpoint.WorkingDir); err == nil {
				destDir = logDir
			}
		}
		if destDir != "" {
