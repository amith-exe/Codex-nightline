...
func (s Store) IsLocal(workingDir string) (bool, string, error) {
	resolved, err := resolveWorkingDir(workingDir)
	if err != nil {
		return false, "", err
	}
...
func (s Store) JournalPath(workingDir string) (string, string, error) {
	isLocal, resolved, err := s.IsLocal(workingDir)
	if err != nil {
		return "", "", err
	}
	if isLocal {
		return filepath.Join(resolved, "flowsync", "journal.md"), resolved, nil
	}

	projectDir, _, err := s.ProjectDir(workingDir)
	if err != nil {
		return "", "", err
	}
...
func (s Store) CheckpointLogDir(workingDir string) (string, error) {
	isLocal, resolved, err := s.IsLocal(workingDir)
	if err != nil {
		return "", err
	}
	if isLocal {
		return filepath.Join(resolved, ".flowsync", "checkpoint-logs"), nil
	}
