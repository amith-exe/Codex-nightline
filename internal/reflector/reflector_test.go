...
func TestClientReflectPipesOllamaStdin(t *testing.T) {
	runner := &fakeRunner{output: "> Useful line\n\nBody."}
	client, err := NewClient(Config{
		Command:    "ollama",
	...
	if runner.command != "ollama" {
		t.Fatalf("command = %q, want ollama", runner.command)
	}
	if len(runner.args) != 2 || runner.args[0] != "run" || runner.args[1] != "qwen2.5-coder:7b" {
		t.Fatalf("args = %#v, want [run qwen2.5-coder:7b]", runner.args)
	}
	if !strings.Contains(runner.stdin, "event excerpt") {
		t.Fatalf("stdin = %q, want containing event excerpt", runner.stdin)
	}
	if response.Body != "> Useful line\n\nBody." {
		t.Fatalf("response body = %q, want expected summary", response.Body)
	}
}
...
func (r *fakeRunner) Run(_ context.Context, command string, args []string, stdin io.Reader) (string, error) {
	r.command = command
