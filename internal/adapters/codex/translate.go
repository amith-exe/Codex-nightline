...
func DecodeHookPayload(data []byte) (HookPayload, error) {
	...
	if err := json.Unmarshal(data, &payload); err != nil {
		return HookPayload{}, fmt.Errorf("decode Codex hook payload: %w", err)
	}
	return payload, nil
}
...
func (t Translator) Translate(payload HookPayload) ([]core.Event, error) {
	...
	default:
		return nil, fmt.Errorf("unsupported Codex hook event %q", payload.HookEventName)
	}
}
...
func (t Translator) preCompact(payload HookPayload, ts time.Time) ([]core.Event, error) {
	event, err := newTextEvent(payload, ts, core.ActorHarness, "pre_compact cause="+compactCause(payload.Trigger))
	if err != nil {
		return nil, err
	}
	return []core.Event{event}, nil
...
func compactCause(trigger string) string {
	if strings.EqualFold(strings.TrimSpace(trigger), "manual") {
		return "manual"
	}
	return "auto"
}
...
func (t Translator) postToolUse(payload HookPayload, ts time.Time) ([]core.Event, error) {
	...
	if correlationID == "" {
		correlationID = "codex:" + payload.ToolName
	}
...
func pathChange(payload HookPayload, op core.FileOp) []core.FileChangePayload {
	...
	return []core.FileChangePayload{{
		...
		Op:     op,
		Source: "codex:" + payload.ToolName,
	}}
}

func patchChanges(payload HookPayload) []core.FileChangePayload {
	patch := firstString(payload.ToolInput, "command", "patch")
	if patch == "" {
		return nil
	}
	seen := map[string]core.FileOp{}
	...
	for scanner.Scan() {
		...
		case strings.HasPrefix(line, "*** Add File: "):
			op = core.FileOpCreate
			path = strings.TrimSpace(strings.TrimPrefix(line, "*** Add File: "))
		case strings.HasPrefix(line, "*** Update File: "):
			op = core.FileOpModify
			path = strings.TrimSpace(strings.TrimPrefix(line, "*** Update File: "))
		case strings.HasPrefix(line, "*** Delete File: "):
			op = core.FileOpDelete
			path = strings.TrimSpace(strings.TrimPrefix(line, "*** Delete File: "))
		}
		if path != "" {
			seen[projectRelativePath(payload.CWD, path)] = op
		}
	}
	...
	sort.Strings(paths)
	changes := make([]core.FileChangePayload, 0, len(paths))
	for _, path := range paths {
		changes = append(changes, core.FileChangePayload{
			Path:   path,
			Op:     seen[path],
			Source: "codex:apply_patch",
		})
	}
	return changes
}
...
func summarizeToolResponse(payload HookPayload) string {
	...
	case "apply_patch":
		changes := fileChanges(payload)
		if len(changes) > 0 {
			paths := make([]string, 0, len(changes))
			for _, change := range changes {
				paths = append(paths, fmt.Sprintf("%s:%s", change.Op, change.Path))
			}
	...
	default:
		...
		} else if keys := objectKeys(payload.ToolResponse); len(keys) > 0 {
...
func toolResultStatus(payload HookPayload) core.ToolResultStatus {
	status := strings.ToLower(firstString(payload.ToolResponse, "status"))
	if status == "error" || status == "failed" || status == "failure" {
		return core.ToolResultError
	}
	return core.ToolResultOK
}
